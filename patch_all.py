# patch_all.py - adds three new builtins to evaluator & compiler
import os

# -------------------- evaluator.py --------------------
with open('evaluator.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the line after the اخرا block (last built-in)
target = None
for i, line in enumerate(lines):
    if "elif node.name == '\\u0627\\u062c\\u0631\\u0627':" in line:
        # insert after the whole block (after the return line)
        target = i + 2  # return subprocess.getoutput(cmd) line + next line
        break

if target:
    indent = '        '
    new_funcs = [
        indent + "elif node.name == '\\u0648\\u0631\\u0648\\u062f\\u06cc_\\u067e\\u0646\\u0647\\u0627\\u0646':\n",
        indent + "    import getpass\n",
        indent + "    return getpass.getpass('')\n",
        indent + "elif node.name == '\\u062e\\u0631\\u0648\\u062c\\u06cc':\n",
        indent + "    import subprocess\n",
        indent + "    cmd = evaluate(node.args[0], env, functions)\n",
        indent + "    return subprocess.getoutput(cmd)\n",
        indent + "elif node.name == '\\u062a\\u0627\\u0631\\u06cc\\u062e_\\u0627\\u0645\\u0631\\u0648\\u0632':\n",
        indent + "    import datetime\n",
        indent + "    return datetime.date.today().strftime('%Y-%m-%d')\n",
    ]
    for j, l in enumerate(new_funcs):
        lines.insert(target + j, l)
    with open('evaluator.py', 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print("evaluator.py updated")
else:
    print("ERROR: Insert point not found in evaluator.py")

# -------------------- compiler.py --------------------
with open('compiler.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# A) Add type info in infer_type
for i, line in enumerate(lines):
    if "if node.name == '\\u062e\\u0637\\u0627_\\u0628\\u062f\\u0647': return 'void'" in line:
        new_types = [
            "            if node.name == '\\u062e\\u0631\\u0648\\u062c\\u06cc': return 'string'\n",
            "            if node.name == '\\u062a\\u0627\\u0631\\u06cc\\u062e_\\u0627\\u0645\\u0631\\u0648\\u0632': return 'string'\n",
            "            if node.name == '\\u0648\\u0631\\u0648\\u062f\\u06cc_\\u067e\\u0646\\u0647\\u0627\\u0646': return 'string'\n",
        ]
        for j, nt in enumerate(new_types):
            lines.insert(i + 1 + j, nt)
        print("infer_type updated")
        break

# B) Add helper C functions after current_time
for i, line in enumerate(lines):
    if 'char* current_time()' in line:
        insert_pos = i + 6  # after the closing '}' line
        helpers = [
            "char* current_date() {\n",
            "    time_t now = time(NULL);\n",
            "    struct tm* t = localtime(&now);\n",
            "    static char buf[11];\n",
            "    strftime(buf, sizeof(buf), \"%Y-%m-%d\", t);\n",
            "    return buf;\n",
            "}\n\n",
            "char* read_pipe(const char* cmd) {\n",
            "    FILE* fp = popen(cmd, \"r\");\n",
            "    if (!fp) return \"\";\n",
            "    static char buf[4096];\n",
            "    size_t n = fread(buf, 1, sizeof(buf)-1, fp);\n",
            "    buf[n] = '\\0';\n",
            "    pclose(fp);\n",
            "    return buf;\n",
            "}\n\n"
        ]
        for j, h in enumerate(helpers):
            lines.insert(insert_pos + j, h)
        print("Helper functions added")
        break

# C) Insert library functions in gen_expr after اخرا
target_expr = None
for i, line in enumerate(lines):
    if "if node.name == '\\u0627\\u062c\\u0631\\u0627':" in line and 'gen_expr' in lines[i-3]:
        target_expr = i + 2
        break
if target_expr:
    lib_expr = [
        "            if node.name == '\\u062e\\u0631\\u0648\\u062c\\u06cc':\n",
        "                cmd = gen_expr(node.args[0], func_name, local_vars, decls)\n",
        "                return f'read_pipe({cmd})'\n",
        "            if node.name == '\\u062a\\u0627\\u0631\\u06cc\\u062e_\\u0627\\u0645\\u0631\\u0648\\u0632':\n",
        "                return 'current_date()'\n",
        "            if node.name == '\\u0648\\u0631\\u0648\\u062f\\u06cc_\\u067e\\u0646\\u0647\\u0627\\u0646':\n",
        "                return '\"<hidden_input>\"'\n",  # placeholder
    ]
    for j, l in enumerate(lib_expr):
        lines.insert(target_expr + j, l)
    print("gen_expr updated")

# D) Insert library functions in gen_stmt after اخرا
target_stmt = None
for i, line in enumerate(lines):
    if "if node.name == '\\u0627\\u062c\\u0631\\u0627':" in line and 'gen_stmt' in lines[i-3]:
        target_stmt = i + 2
        break
if target_stmt:
    lib_stmt = [
        "            if node.name == '\\u062e\\u0631\\u0648\\u062c\\u06cc':\n",
        "                cmd = gen_expr(node.args[0], func_name, local_vars, decls)\n",
        "                return f'printf(\"%s\", read_pipe({cmd}))'\n",
        "            if node.name == '\\u062a\\u0627\\u0631\\u06cc\\u062e_\\u0627\\u0645\\u0631\\u0648\\u0632':\n",
        "                return 'printf(\"%s\", current_date());'\n",
        "            if node.name == '\\u0648\\u0631\\u0648\\u062f\\u06cc_\\u067e\\u0646\\u0647\\u0627\\u0646':\n",
        "                return 'printf(\"<hidden>\\n\");'\n",
    ]
    for j, l in enumerate(lib_stmt):
        lines.insert(target_stmt + j, l)
    print("gen_stmt updated")

# E) Fix اجرا to not print exit code (change the gen_stmt version)
for i, line in enumerate(lines):
    if "return f'system({cmd})'" in line and 'gen_stmt' in lines[i-3]:
        lines[i] = "                return f'system({cmd});'\n"
        print("Fixed اجرا to not print exit code")
        break

with open('compiler.py', 'w', encoding='utf-8') as f:
    f.writelines(lines)
print("compiler.py updated")