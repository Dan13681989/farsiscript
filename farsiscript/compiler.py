...
def compile_to_c(code, output_c_file):
    ...
    def infer_type(node, func_name=None):
        ...
        if isinstance(node, CallNode):
            ...
            if node.name == '\u0639\u062f\u062f_\u062a\u0635\u0627\u062f\u0641\u06cc': return 'int'
            if node.name == '\u0627\u062c\u0631\u0627': return 'int'
            if node.name == '\u062e\u0631\u0648\u062c\u06cc': return 'string'
            if node.name == '\u062a\u0627\u0631\u06cc\u062e_\u0627\u0645\u0631\u0648\u0632': return 'string'
            if node.name == '\u0648\u0631\u0648\u062f\u06cc_\u067e\u0646\u0647\u0627\u0646': return 'string'
        ...
        if isinstance(node, DotNode):
            if isinstance(node.obj, ThisNode) and func_name and '_' in func_name:
                cls_name = func_name.split('_')[0]
                constr_name = f'{cls_name}_\u062c\u062f\u06cc\u062f'
                field_type = param_types.get((constr_name, node.attr), 'double')
                return field_type
            return 'double'
        if isinstance(node, ThisNode): return 'self_ptr'
        return 'double'
    ...
    def find_calls(node, func_name=None):
        if isinstance(node, CallNode):
            fn = node.name
            if '.' in fn:
                obj_name, method = fn.split('.')
                if obj_name in var_info and var_info[obj_name].get('type') == 'class_obj':
                    cls_name = var_info[obj_name]['class_name']
                    safe_fn = f'{cls_name}_{method}'
                    if safe_fn in functions:
                        f = functions[safe_fn]
                ...
                constr_name = f'{fn}_\u062c\u062f\u06cc\u062f'
                if constr_name in functions:
                    f = functions[constr_name]
                    if isinstance(f, FunctionNode):
                        for p, a in zip(f.params, node.args):
                            t = infer_type(a, func_name)
                            key = (constr_name, p)
            ...
            find_calls(node.expr, func_name)
        elif isinstance(node, list):
    ...
    for fnode in functions.values():
        if isinstance(fnode, FunctionNode):
    ...
    for cls_name, cls_node in classes.items():
        ...
        for field in fields:
            t = param_types.get((f'{cls_name}_\u062c\u062f\u06cc\u062f', field), 'double')
            c_code += f'    {"char*" if t=="string" else "double"} {field};\n'
    ...
    def gen_expr(node, func_name, local_vars, decls):
        ...
        if isinstance(node, VarNode):
            if func_name and (func_name, node.name) in param_types:
                return node.name
        ...
        if isinstance(node, CallNode):
            ...
            if node.name == '\u062e\u0637\u0627_\u0628\u062f\u0647':
                arg = gen_expr(node.args[0], func_name, local_vars, decls)
                return arg
            # --- Standard library (gen_expr) ---
            if node.name == '\u0632\u0645\u0627\u0646_\u0627\u06a9\u0646\u0648\u0646':
                return 'current_time()'
            if node.name == '\u0639\u062f\u062f_\u062a\u0635\u0627\u062f\u0641\u06cc':
    ...
    def gen_stmt(node, func_name, local_vars, decls):
        ...
        if isinstance(node, CallNode):
            ...
            # --- Standard library (gen_stmt) ---
            if node.name == '\u0632\u0645\u0627\u0646_\u0627\u06a9\u0646\u0648\u0646':
                return 'printf("%s", current_time());'
            if node.name == '\u0639\u062f\u062f_\u062a\u0635\u0627\u062f\u0641\u06cc':
                low = gen_expr(node.args[0], func_name, local_vars, decls)
                high = gen_expr(node.args[1], func_name, local_vars, decls)
    ...
    for fn, fnode in functions.items():
        ...
        prototype_list.append(f'{ret} {fn}({", ".join(pdecls)});')
    ...
    for fn, fnode in functions.items():
        ...
        has_return = any(isinstance(s, ReturnNode) or (isinstance(s,list) and any(isinstance(ss,ReturnNode) for ss in s)) for s in fnode.body)
        ret = 'double' if has_return else 'void'
        c_code += f'{ret} {fn}({", ".join(pdecls)}) {{\n'
        c_code += gen_func_body(fnode.body, func_name=fn)
        c_code += '\n}\n\n'

    c_code += 'void gregorian_to_jalali_c(int gy, int gm, int gd, int* jy, int* jm, int* jd) {\n'
    c_code += '    int g_days_in_month[] = {31,28,31,30,31,30,31,31,30,31,30,31};\n'
    c_code += '    int j_days_in_month[] = {31,31,31,31,31,31,30,30,30,30,30,29};\n'
    c_code += '    int gy2 = gy - 1600;\n'
    c_code += '    int gm2 = gm - 1;\n'
    c_code += '    int g_day_no = 365 * gy2 + (gy2 + 3) / 4 - (gy2 + 99) / 100 + (gy2 + 399) / 400;\n'
    c_code += '    for (int i = 0; i < gm2; i++) g_day_no += g_days_in_month[i];\n'
    c_code += '    if (gm2 > 1 && ((gy % 4 == 0 && gy % 100 != 0) || (gy % 400 == 0))) g_day_no++;\n'
    c_code += '    g_day_no += gd - 1;\n'
    c_code += '    int j_day_no = g_day_no - 79;\n'
    c_code += '    int j_np = j_day_no / 12053;\n'
    c_code += '    j_day_no %= 12053;\n'
    c_code += '    int jy_temp = 979 + 33 * j_np + 4 * (j_day_no / 1461);\n'
    c_code += '    j_day_no %= 1461;\n'
    c_code += '    if (j_day_no >= 366) {\n'
    c_code += '        jy_temp += (j_day_no - 1) / 365;\n'
    c_code += '        j_day_no = (j_day_no - 1) % 365;\n'
    c_code += '    }\n'
    c_code += '    int ji;\n'
    c_code += '    for (ji = 0; ji < 11 && j_day_no >= j_days_in_month[ji]; ji++) j_day_no -= j_days_in_month[ji];\n'
    c_code += '    *jm = ji + 1;\n'
    c_code += '    *jd = j_day_no + 1;\n'
    c_code += '    *jy = jy_temp;\n'
    c_code += '}\n\n'
    c_code += 'char* shamsi_date() {\n'
    c_code += '    time_t t = time(NULL);\n'
    c_code += '    struct tm* tm = localtime(&t);\n'
    c_code += '    int jy, jm, jd;\n'
    c_code += '    gregorian_to_jalali_c(tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, &jy, &jm, &jd);\n'
    ...
    c_code += '    srand(time(NULL));\n'
    c_code += gen_func_body(statements, func_name=None)
    c_code += '\n    return 0;\n}\n'
